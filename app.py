...
app = Flask(__name__)
db_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'assets_v8.db')
...
<body>
    <div class="ai-section text-center">
        <div class="container">
            <h2 class="fw-bold text-white mb-4">🤖 Gemini AI 智慧助理已上線</h2>
            <div class="row justify-content-center">
                <div class="col-lg-8">
                    <form action="/smart_input" method="POST" class="ai-input-wrapper">
                        <input type="text" name="raw_text" placeholder="輸入：買 2330 1張 / 中信轉台新 3000 / 晚餐 -150" required>
    ...
    <div class="container mt-5">
        <div class="row">
            <div class="col-md-4">
                <div class="card p-4">
                    <h6 class="text-muted">總資產估值</h6>
                    <h2 class="text-primary fw-bold">${{ "{:,.0f}".format(total_cash + total_stock) }}</h2>
                    <hr style="border-color:#333">
                    <div class="d-flex justify-content-between"><span>現金</span><span>${{ "{:,.0f}".format(total_cash) }}</span></div>
                    <div class="d-flex justify-content-between"><span>股票</span><span class="text-success">${{ "{:,.0f}".format(total_stock) }}</span></div>
                </div>
            </div>
            <div class="col-md-8">
                <div class="card p-4">
                    <h6 class="fw-bold mb-3">資產明細一覽</h6>
                    <table class="table table-dark table-hover">
                        <thead><tr><th>項目</th><th>分類</th><th>金額</th><th>操作</th></tr></thead>
                        <tbody>
                            {% for c in cash_items %}
                            <tr><td>{{ c[1] }}</td><td>現金</td><td>${{ "{:,.0f}".format(c[2]) }}</td><td><a href="/delete/{{ c[0] }}" class="text-danger">刪除</a></td></tr>
                            {% endfor %}
                            {% for s in stocks %}
                            <tr><td>{{ s.symbol }}</td><td>股票</td><td class="text-success">${{ "{:,.0f}".format(s.value) }}</td><td>-</td></tr>
                            {% endfor %}
                        </tbody>
                    </table>
                </div>
...
def index():
    ...
    for symbol, shares in stocks_raw:
        if shares > 0:
            try:
                price = float(yf.Ticker(symbol).fast_info.get('last_price', 0))
                val = shares * price
                total_stock += val
                stock_list.append({'symbol': symbol, 'value': val})
            except: stock_list.append({'symbol': symbol, 'value': 0})
    conn.close()
